...
export function MulitNodeGrid() {
  ...
  const { data: mccBalanceData, isLoading: mccLoading } = useBalance({
    address: MULTINODE_CLAIM_CONTRACT,
    token: MCC_CONTRACT,
    chainId: chain?.id,
  });

  const { data: currentAddressMCC } = useBalance({
    address,
    token: MCC_CONTRACT,
    chainId: chain?.id,
  });

  const mccBalanceFormatted = mccBalanceData?.formatted;

  const { write } = useContractWrite(chain?.id, {
    mode: 'recklesslyUnprepared',
    address: MULTINODE_CLAIM_CONTRACT,
    abi: MultiNodeContractAbi,
    functionName: 'claimDividendsMulti',
    onSuccess(data) {
      gtag.event({
  ...
  const claimNodeRewards = useCallback(
    (selectedNodes?: MultiNodeType[]) => {
      try {
        ...
        if (
          !mccLoading &&
          // @ts-ignore
          walletNodeStats?.available > parseInt(mccBalanceFormatted)
        ) {
        ...
        const gasLimit =
          RECOMMENDED_MULTI_GAS *
          nodeIds?.length *
          generateChainGasMultiple(chain?.id);

        // @ts-ignore
        write({
          recklesslySetUnpreparedArgs: [[...nodeIds], { gasLimit }],
        });
      } catch (error: any) {
        console.error(error);
      }
    },
    [chain?.id, mccBalanceFormatted, walletNodeStats?.available, write]
  );
  ...
  const { data: priceData } = useSWR('/api/mcc-price', (url: string) =>
    request(url)
  );
              ...
              <div className="flex flex-col items-end flex-shrink-0 mt-4 sm:flex-row md:mt-0 md:ml-4">
                ...
                <>
                  <button
                    type="button"
                    onClick={(e) => {
                      claimNodeRewards(claimableNodes);
                    }}
                    disabled={
                      !walletNodeStats?.available && write !== undefined
                    }
                    className={clsx(
                      !walletNodeStats?.available && write !== undefined
                          ...
                          <>
                            ...
                            {abbreviateNumber(
                              (item?.id === 'mccBalance'
                                ...
                                ?.replaceAll(',', '') *
                                priceData?.[
                                  `${generateChainNameIdentifier(chain?.id)}`
                                ],
                              2
                            )}{' '}
                            USD
              ...
              <div className="relative flex flex-col items-center ml-1 text-gray-200 group">
                ...
                <div className="absolute bottom-0 flex-col items-center hidden w-24 mb-6 text-center group-hover:flex">
                  <span className="relative z-10 p-2 text-xs leading-none text-white whitespace-no-wrap bg-black rounded shadow-lg">
                    ...
                    of MultiNode claims. Gas prices costs for your claim may
                    vary due to current gas and price fluctuations.
                  </span>
